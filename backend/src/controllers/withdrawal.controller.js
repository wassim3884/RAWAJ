...
// Minimum payout amount — enforced here (not just in the UI) since the
// frontend can't be trusted as the source of truth for money movement.
...
async function requestWithdrawal(req, res) {
  ...
  if (Number(amount) < MIN_WITHDRAWAL_AMOUNT) {
    return res.status(400).json({ error: `الحد الأدنى لطلب السحب هو ${MIN_WITHDRAWAL_AMOUNT} د.ج.` });
  }
