...
async function createOrder(req, res) {
  ...
  try {
    ...
    // Only affiliates who were approved to promote this product may submit a lead for it —
    // unless the product doesn't require approval.
    if (product.requires_approval) {
      const approval = await client.query(
        `SELECT status FROM affiliate_product_requests WHERE affiliate_id = $1 AND product_id = $2`,
        [req.user.id, productId]
      );
      if (!approval.rows.length || approval.rows[0].status !== 'approved') {
        throw new Error('You are not approved to promote this product yet.');
      }
    }
