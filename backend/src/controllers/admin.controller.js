...
async function updateSiteSetting(req, res) {
  ...
  try {
    // site_settings.value is JSONB NOT NULL (see schema.sql). A plain JS
    // string (e.g. a Telegram URL, or raw page HTML) sent as-is is NOT
    // valid JSON on its own — Postgres rejected it outright, which is why
    // saving ever failed here ("Failed to save link." / any plain-text
    // setting). JSON.stringify() makes any value (string, object, array,
    // number) valid JSON before it reaches the JSONB column; reading it
    // back via getSiteSetting round-trips correctly either way since pg
    // auto-decodes JSONB.
    const result = await db.query(
