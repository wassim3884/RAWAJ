...
// Password reset links stay valid for 1 hour — matches the copy in
// utils/email.js's passwordResetTemplate ("صالح لمدة ساعة واحدة فقط").
const PASSWORD_RESET_EXPIRY_MS = 60 * 60 * 1000;

// SHA-256 hash of the raw reset token. Only the hash is ever stored in the
// database (users.password_reset_token) — the raw token exists only in the
// email link, the same pattern bcrypt uses for passwords: possession of the
// DB doesn't hand out working reset links.
...
async function forgotPassword(req, res) {
  ...
  // Generic response used for every outcome below (user found or not,
  // inactive account, or an unexpected error) so the endpoint never reveals
  // whether a given email has an account on the platform.
  ...
  try {
    ...
    if (user && user.is_active) {
      ...
      // Best-effort send: a failed email shouldn't leak account existence
      // via a different response, so errors are only logged here.
      sendEmail(
        user.email,
        'إعادة تعيين كلمة المرور - رواج',
        passwordResetTemplate(resetUrl, user.full_name)
