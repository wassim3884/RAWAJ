...
import { LayoutDashboard, Users, Package, Wallet, Settings, Tag, Crown, Truck, Plus, Send } from 'lucide-react';
  ...
  return (
    <div className="flex min-h-[80vh] flex-col md:flex-row">
      ...
      <div className="flex-1 p-6">
        <h1 className="mb-6 text-2xl font-bold">Wholesale Catalog</h1>

        <div className="card mb-8">
          <p className="mb-2 font-semibold">Telegram Channel</p>
        ...
        <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
          <div className="flex gap-2">
            ...
            <button onClick={load} className="btn-outline !py-2 text-sm">Search</button>
          </div>
        ...
        {showForm && (
          <form onSubmit={handleSubmit} className="card mb-8 space-y-4">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <Field label="Image URL"><input value={form.imageUrl} onChange={(e) => setForm({ ...form, imageUrl: e.target.value })} className="input" /></Field>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {products.map((p) => (
            <div key={p.id} className="card">
              ...
              <p className="text-sm text-slate-500">${Number(p.wholesale_price).toFixed(2)} · MOQ {p.min_order_quantity}</p>
              {p.source_notes && <p className="mt-1 text-xs italic text-slate-400">{p.source_notes}</p>}
              <button onClick={() => deleteProduct(p.id)} className="mt-3 text-sm text-red-500">Delete</button>
            </div>
          ))}
          {!products.length && <p className="text-slate-400">No wholesale products yet.</p>}
        </div>
      </div>
