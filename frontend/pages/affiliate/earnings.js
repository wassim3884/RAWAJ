...
// a separate validation rule.
const PHONE_REGEX = /^0[567]\d{8}$/;
const MIN_WITHDRAWAL_AMOUNT = 500; // matches backend/src/controllers/withdrawal.controller.js
...
export default function AffiliateEarnings() {
  ...
  const submitWithdrawal = async (e) => {
    e.preventDefault();
    if (submitting) return; // guard against double-click submitting twice
    setSubmitting(true);
    try {
      if (method === 'products') {
        ...
        if (selectedProductsValue < MIN_WITHDRAWAL_AMOUNT) {
          toast.error(`الحد الأدنى لطلب السحب هو ${formatDZD(MIN_WITHDRAWAL_AMOUNT)}.`);
          setSubmitting(false);
          return;
        }
        ...
        if (Number(amount) > Number(stats?.profile?.balance || 0)) {
          toast.error('المبلغ المطلوب أكبر من رصيدك المتاح.');
          setSubmitting(false);
          return;
        }
        if (METHOD_NEEDS_PHONE.has(method) && !PHONE_REGEX.test(payoutContact)) {
          toast.error('رقم هاتف Flexy غير صالح. يجب أن يبدأ بـ 05 أو 06 أو 07 ويتكوّن من 10 أرقام.');
          setSubmitting(false);
          return;
        }
          ...
          <div className="card">
            ...
            <form onSubmit={submitWithdrawal} className="space-y-3">
              ...
              {method === 'products' ? (
                ...
                <>
                  <div>
                    <input type="number" step="0.01" min={MIN_WITHDRAWAL_AMOUNT} required placeholder="المبلغ (د.ج)" value={amount} onChange={(e) => setAmount(e.target.value)}
                      className="w-full rounded-xl border border-slate-200 px-4 py-2.5 dark:border-slate-700 dark:bg-slate-900" />
                    <p className="mt-1 text-xs text-slate-400">الحد الأدنى للسحب: {formatDZD(MIN_WITHDRAWAL_AMOUNT)}</p>
                  </div>
                  <input
                    required
                    type={METHOD_NEEDS_PHONE.has(method) ? 'tel' : 'text'}
                    placeholder={METHOD_NEEDS_PHONE.has(method) ? `${METHOD_LABELS[method]} — رقم الهاتف (05XXXXXXXX)` : 'رقم الهاتف / معرّف الحساب للدفع'}
                    value={payoutContact}
                    onChange={(e) => setPayoutContact(e.target.value)}
                    className="w-full rounded-xl border border-slate-200 px-4 py-2.5 dark:border-slate-700 dark:bg-slate-900"
                  />
                  {METHOD_NEEDS_PHONE.has(method) && payoutContact && !PHONE_REGEX.test(payoutContact) && (
                    <p className="text-xs text-red-500">رقم الهاتف يجب أن يبدأ بـ 05 أو 06 أو 07 ويتكوّن من 10 أرقام.</p>
                  )}
                </>
              )}
